...
.PARAMETER PrivateLinkScopeId
   ResourceId of the private link scope used by the Azure Connected machine Agent. 
...
Function Update-ArcAgentVersion {
    ...
    if ($LocalAgentVersion -lt $NetworkShareVersion) {
        ...
        Write-Log -msg "Newer Agent version found in network share folder: $($NetworkShareVersion.ToString()) , local version is $($LocalAgentVersion.ToString()). Executing update ..." -msgtype WARNING
...
if (($ArcOnboardingData.AzureVM -eq $false) -and ($ArcOnboardingData.ArcCompatible -eq $false)) {
    Write-Log -msg "Machine doesn't meet the minimun requirements for Azure Arc: Windows PowerShell 4.0 and NET Framework 4.6, or it is an Azure VM." -msgtype ERROR
    Send-ArcData -Data $ArcOnboardingData -Path "$LoggingNetworkPath\$($env:COMPUTERNAME).xml"
    Write-Log -msg "End of the Azure Arc Onboarding process..." -msgtype INFO
