...
#Input section
$Reportfolders = @("\\server\AzureArcOnboard\AzureArcLogging") # Path to the network share(s) and folder(s) with XMLs logs
...
foreach ($Reportfolder in $Reportfolders) {
    $file = Get-ChildItem -Recurse -Filter *.xml -Path $Reportfolder -File
    $files += $file
}
