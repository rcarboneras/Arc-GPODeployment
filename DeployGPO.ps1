...
.PARAMETER PrivateLinkScopeId
  Resource id of the private link scope used by the Agent
  e.g. /subscriptions/dec8cb97-b3f9-4120-b71a-514ae10100fc/resourceGroups/RG-AzureArc/providers/Microsoft.HybridCompute/privateLinkScopes/arcpls-we-arcdemo
...
#Add Private Link Scope information if neccessary

if ($PSBoundParameters.ContainsKey('PrivateLinkScopeId')) {
    Write-Host "`nPrivateLinkScopeId was set. Adding private link scope $PrivateLinkScopeId to the scheduled task ..." -ForegroundColor Green

    try {
        $xmlcontent = Get-Content -Path $ScheduledTaskfile -ErrorAction Stop
        ($xmlcontent -replace "EnableAzureArc.ps1 -ReportServerFQDN $ReportServerFQDN", "EnableAzureArc.ps1 -ReportServerFQDN $ReportServerFQDN -PrivateLinkScopeId $PrivateLinkScopeId") | Out-File $ScheduledTaskfile -Encoding utf8 -Force -ErrorAction Stop
        Write-Host "Private link scope information was successfully added to the scheduled task" -ForegroundColor Green
    }
    catch { Write-Host "Could not add Private link scope Information:`n$(($_.Exception).Message)" -ForegroundColor Red ; exit }
...
try {
    ...
    New-GPO -Name $GPONamewithTimestamp -DomainName $DomainFQDN -ErrorAction Stop
...
try {
    Import-GPO -Path $BackupPath -TargetName $GPONamewithTimestamp -Domain $DomainFQDN -BackupId $Backupid -ErrorAction Stop
